...
# --- printing helper: keeps debug logic but suppresses prints unless verbose=True
def _debug_print(verbose: bool, *args, **kwargs):
    if verbose:
        print(*args, **kwargs)
...
def timeout_handler(signum, frame):
    _debug_print(False, "alarm went off")  # never prints unless you flip verbose at callsites
    raise TimeoutException
signal.signal(signal.SIGALRM, timeout_handler)


def module_from_string(name: str, code: str):
...
def compile_and_get_func(program, which_type, method_name, timeout, debug, verbose):
    signal.alarm(timeout)
    try:
        tmp_sol = module_from_string("tmp_sol", program)
        ...
        signal.alarm(0)
        if debug:
            _debug_print(verbose, f"compilation error = {e}")
        return False
    signal.alarm(0)
    ...
    try:
        ...
        e = sys.exc_info()
        if debug:
             _debug_print(verbose, f"unable to get function error = {e}")
        return False
    return method

def synthesize_cb_code(raw_code, debug=False, verbose=False):
    sol = "import sys\nimport time\nimport itertools\nfrom itertools import accumulate, product, permutations, combinations\nimport collections\nfrom collections import Counter, OrderedDict, deque, defaultdict, ChainMap\nfrom functools import lru_cache\nimport math\nfrom math import sqrt, sin, cos, tan, ceil, fabs, floor, gcd, exp, log, log2\nimport fractions\nfrom typing import List, Tuple\nimport numpy as np\nimport random\nimport heapq\nfrom heapq import *\n"
    if debug:
        _debug_print(verbose, f"loading test code = {datetime.now().time()}")
    sol += raw_code
    return sol

def synthesize_std_code(raw_code, debug=False, verbose=False):
    ...
    if debug:
        _debug_print(verbose, f"sol = {sol}")
        _debug_print(verbose, f"sol2 = {sol2}")
...
def print_debug_info(inputs, outputs, exec_outputs, verbose=False):
    ...
    if not isinstance(inputs, list):
        _debug_print(verbose, f"exec output = {exec_outputs}, test outputs = {outputs}, inputs = {inputs.replace(nl,' new-line ')}, {type(inputs)}, {exec_outputs == [outputs]}")
    else:
        _debug_print(verbose, f"exec output = {exec_outputs}, test outputs = {outputs}, inputs = {inputs}, {type(inputs)}, {exec_outputs == [outputs]}")
...
def compare_std_results(exec_outputs, outputs, debug=False, verbose=False):
    ...
    try:
        ...
        if debug:
            _debug_print(verbose, f"Failed check1 exception = {e}")
        pass
    if tmp_result:
        return True
    ...
    try:
        ...
        if debug:
            _debug_print(verbose, f"Failed check2 exception = {e}")
        pass
    if tmp_result:
        return True
    ...
    try:
        ...
        if debug:
            _debug_print(verbose, f"Failed check3 exception = {e}")
        pass
    if tmp_result:
        return True
    ...
    try:
        ...
        if debug:
            _debug_print(verbose, f"Failed check4 exception = {e}")
    if tmp_result:
        return True
    ...
    try:
        ...
        if debug:
            _debug_print(verbose, f"Failed check5 exception = {e}")
    ...
    try:
        ...
        if debug:
            _debug_print(verbose, f"Failed check6 exception = {e}")
...
# --- New: minimal enum resolver ---
def _resolve_code_type(fn_name: str | None) -> CODE_TYPE:
        ...
        out.append(
            {
                ...
                "ground_truth_output": outputs_list[idx],
                "status": status,
            }
        )
    return out
...
def check_correctness(
    ...
    *,
    ...
    p = multiprocessing.Process(target=_runner, args=(shared_res,))
    p.start()
    p.join()
    if p.is_alive():
        p.kill()
...
def taco_data_converter(taco_hf_instance):
    question = taco_hf_instance["prompt"][0]["content"]
    test_data = json.loads(taco_hf_instance["reward_model"]["ground_truth"])
...
def calculate_score(code_generation, test_inputs, test_outputs, fn_name) -> float:
    """
    Returns the success rate in [0.0, 1.0] for the given code against the tests.
    Uses check_correctness(), which returns a list of dicts:
      {"test_input": ..., "predicted_output": ..., "ground_truth_output": ..., "status": ...}
    """
    results = check_correctness(
        ...
        test_outputs=test_outputs,
        fn_name=fn_name,
        # keep defaults: debug=True, verbose=False
    )
