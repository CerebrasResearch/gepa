from gepa.adapters.verifier_adapter import VerifierAdapter
from gepa.adapters.verifier_adapter.prompts import SYSTEM_PROMPT_DEEPSEEK_GRM, USER_PROMPT_DEEPSEEK_GRM_2_RESPONSES_SHORTENED


def init_dataset(anymaths_dset_name: str = "dataset_code_100"):
    import random
    ...
    match anymaths_dset_name:
        case "dataset_code_100":
            train_load_dataset = load_dataset("yashm-cerebras/coding_dataset_train_100", split="train", token=token)
            ...
            test_load_dataset = load_dataset("yashm-cerebras/coding_dataset_test_100", split="train", token=token)
            for item in test_load_dataset:
                answer = item["answer"]
                question = item["question"]
                response1 = item["response1"]
                response2 = item["response2"]
                test_split.append({"input": question,
                        "additional_context": {
                        "response1": response1,
                        "response2": response2,
                        "answer": answer,
        ...
        case "dataset_complete":
            train_load_dataset = load_dataset("yashm-cerebras/coding_dataset_complete", split="train", token=token)
            test_load_dataset = load_dataset("yashm-cerebras/coding_dataset_complete", split="test", token=token)
            ...
            random.Random(0).shuffle(train_split)
            train_split = train_split[:1000]  # limit to 1000 for faster training
    ...
    trainset = train_split[: 3*(len(train_split) // 4)]
    valset = train_split[3*(len(train_split) // 4) :]
    testset = test_split
...
if __name__ == "__main__":
    ...
    # vllm serve c --port 8000 
    parser = argparse.ArgumentParser()
    parser.add_argument("--anymaths_dset_name", type=str, default="dataset_code_100")
    parser.add_argument("--train_size", type=int, default=10, help="The size of the training set to use.")
    parser.add_argument("--val_size", type=int, default=10, help="The size of the validation set to use.")
    parser.add_argument("--test_size", type=int, default=10, help="The size of the test set to use.")
    parser.add_argument("--base_lm", type=str, default="hosted_vllm/Qwen/Qwen3-8B")
    parser.add_argument("--use_api_base", action="store_true", help="Use API base URL")
    parser.add_argument("--api_base_url", type=str, default="http://127.0.0.1:8000/v1")
    parser.add_argument(
        "--reflection_lm", type=str, default="hosted_vllm/Qwen/Qwen3-8B", help="The name of the reflection LM to use."
    )
    ...
    _reflection["api_base"] = api_reflection
    _reflection["api_key"] = "EMPTY"  
    ...
    optimized_results = optimize(
        seed_candidate={
            "system_prompt": SYSTEM_PROMPT_DEEPSEEK_GRM,
            "user_prompt_template": USER_PROMPT_DEEPSEEK_GRM_2_RESPONSES_SHORTENED
        },
        trainset=trainset,
        valset=valset,
        adapter=VerifierAdapter(model=base_lm, reflection_model=reflection_lm_name, api_base=api_base, max_litellm_workers=max_litellm_workers, reflection_api_base=api_reflection),
